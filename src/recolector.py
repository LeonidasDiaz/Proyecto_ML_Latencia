...
def obtener_latencia():
    """
    Mide la latencia como el tiempo de respuesta HTTP del servidor local.
    """
    try:
        respuesta = requests.get('http://127.0.0.1:8080/', timeout=2)
        # Convertir timedelta a milisegundos
        latencia_ms = respuesta.elapsed.total_seconds() * 1000
        return round(latencia_ms, 2)
    except Exception:
        return 0.0
